...
+ (void)deleteAllForCacheDir:(NSString*)dir
{
    NSFileManager* fileManager = [NSFileManager defaultManager];
    NSArray *fileList = [fileManager contentsOfDirectoryAtPath:[self GetDirectoryForCaches:dir] error:nil];
    for (NSString* filename in fileList) {
        if ([filename isEqualToString:@"KCacheTag_ChapterInfo.txt"] ||[filename isEqualToString:@"KCacheTag_Contentinfo.txt"] ||[filename isEqualToString:@"KCacheTag_PlayInfo.txt"] || [filename isEqualToString:@"kCacheTag_NoteFileName.txt"] || [filename isEqualToString:@"KCacheTag_PersonCenter.txt"]  ) {
            continue;
        }else{
...
+ (BOOL)copyFile:(NSString *)path ToDestinationPath:(NSString *)desPath {
    if ([[NSFileManager defaultManager]fileExistsAtPath:desPath]) {
        [[NSFileManager defaultManager]removeItemAtPath:desPath error:NULL];
    }
    return [[NSFileManager defaultManager]copyItemAtPath:path toPath:desPath error:NULL];
}
